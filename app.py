...
def main():
    ...
    def connect_event_listeners(ui_components):
        ...
        # State này không còn cần thiết vì link HTML được tạo trực tiếp
        ...
        unified_analysis_outputs = [
            ...
            ui["view_full_video_html"] # Output mới cho link HTML
        ]
        ...
        def on_transcript_select_wrapper(state, evt: gr.SelectData):
            if not isinstance(evt, gr.SelectData):
                return None, None, "Lỗi: Sự kiện không hợp lệ.", "", None, "", "0.0", "", None
            return handlers.on_transcript_select(state, evt)
        ...
        # Kết nối đúng cho việc thêm từ transcript
